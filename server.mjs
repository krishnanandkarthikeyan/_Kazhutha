...
const savedResults = [];
const SUITS = ['S','H','C','D'];
const classic = {name:'Classic Kerala',aceStart:true,firstTrump:false,cutEnds:true,trumpWins:false,forceTrump:false,winnerLeads:true,collectCutter:false,allow2:true,allow3:true};
const trump = {...classic,name:'Trump-on-First-Vettu',firstTrump:true,cutEnds:false,trumpWins:true,forceTrump:true};
...
function createGame(seats,rules){const cards=shuffle(deck()),hands=seats.map(()=>[]);cards.forEach((c,i)=>hands[i%seats.length].push(c));const starter=rules.aceStart?hands.findIndex(h=>h.some(c=>c.id==='S14')):randomInt(seats.length);return{id:hex(24),players:seats.map((p,i)=>({...p,id:i})),hands,rules:{...rules},currentPlayer:starter,leader:starter,activePlayers:seats.map((_,i)=>i),safePlayers:[],trick:[],leadSuit:null,trumpSuit:null,discarded:[],history:[],voids:seats.map(()=>[]),known:seats.map(()=>[]),opening:true,round:1,moves:0,status:'playing',loser:null,result:null,playedCounts:seats.map(()=>0),vettuCounts:seats.map(()=>0),repetitions:{},lastEvent:{type:'start',text:'The Ace of Spades holder opens the table and may play any card.'}}}
function legal(g,seat=g.currentPlayer){const hand=g.hands?.[seat]||g.hand;if(g.status!=='playing')return[];if(g.opening&&g.rules.aceStart){if(!hand.some(c=>c.id==='S14'))return[];return hand;}if(g.leadSuit){const follow=hand.filter(c=>c.suit===g.leadSuit);if(follow.length)return follow;if(g.rules.forceTrump&&g.trumpSuit&&g.round>1){const tr=hand.filter(c=>c.suit===g.trumpSuit);if(tr.length)return tr}}return hand}
function winner(trick,lead,trumpSuit,rules){const tr=rules.trumpWins&&trumpSuit?trick.filter(x=>x.card.suit===trumpSuit):[];const pool=tr.length?tr:trick.filter(x=>x.card.suit===lead);return pool.reduce((a,b)=>!a||b.card.rank>a.card.rank?b:a,null)}
function nextActive(g,seat,excluded=[]){for(let n=1;n<=g.players.length;n++){const i=(seat+n)%g.players.length;if(g.activePlayers.includes(i)&&!excluded.includes(i))return i}return-1}
function play(g,seat,cardId){if(g.status!=='playing'||g.result)throw fail('Wait for the next trick.',409);if(seat!==g.currentPlayer)throw fail('It is not your turn.',409);if(!legal(g,seat).some(c=>c.id===cardId))throw fail('That card is not legal right now.',409);const a=clone(g),hand=a.hands[seat],idx=hand.findIndex(c=>c.id===cardId);if(idx<0)throw fail('That card is not in your hand.',409);const card=hand.splice(idx,1)[0];const offSuit=!!a.leadSuit&&card.suit!==a.leadSuit;const cut=offSuit&&a.round>1; // First round explicitly has no Vettu.
if(offSuit&&!a.voids[seat].includes(a.leadSuit))a.voids[seat].push(a.leadSuit);
if(cut&&a.rules.firstTrump&&!a.trumpSuit)a.trumpSuit=card.suit;
if(cut)a.vettuCounts[seat]++;
if(!a.leadSuit)a.leadSuit=card.suit;
a.known[seat]=a.known[seat].filter(c=>c.id!==cardId);a.opening=false;a.trick.push({player:seat,card,cut});a.history.push({player:seat,card,cut,round:a.round});a.moves++;a.playedCounts[seat]++;a.lastEvent={type:cut?'vettu':'play',player:seat,text:cut?`${a.players[seat].name} plays Vettu!`:`${a.players[seat].name} plays ${cardId}`};
if((cut&&a.rules.cutEnds)||a.trick.length===a.activePlayers.length){const w=winner(a.trick,a.leadSuit,a.trumpSuit,a.rules),hasCut=a.trick.some(x=>x.cut),cutter=a.trick.find(x=>x.cut)?.player;let collector=hasCut?(a.rules.collectCutter?cutter:w.player):null;if(collector===null&&a.activePlayers.every(i=>a.hands[i].length===0))collector=w.player;a.result={winner:w.player,collector,cut:hasCut,cards:a.trick.map(x=>x.card)}}else a.currentPlayer=nextActive(a,seat,a.trick.map(x=>x.player));return a}
